...
from TorCastML_v2_probs_only import TorCastML_v2_probs_only
...
#==============================================================================================================
#Overall Settings
MODEL = TorCastML_v2_probs_only() #define the model to train
...
def main_task(start_epoch, model_name, model):
    ...
    while (not CONVERGED):
        ...
        do_epoch(model, train_data_loader, DEVICE, optimizer, loss_prob)
        epoch_no += 1
        epoch_end_time = time.time()
        epoch_duration = epoch_end_time - epoch_start_time

        #TEST
        acc, prec, rec, f1, tp, fp, tn, fn = do_test(model, test_data_loader, DEVICE)
        accs.append(acc)
        precs.append(prec)
        recs.append(rec)
        f1s.append(f1)
        ...
        #log metrics
        print("e" + str(epoch_no) + " took " + str((round(epoch_duration % 3600)) // 60) + ":" + str(round(epoch_duration % 60)) + " - " + str(round(acc, 4)) + "," + str(round(prec, 4)) + "," + str(round(rec, 4)) + "," + str(round(f1, 4)) + " | " + str(tp) + "," + str(fp) + "," + str(tn) + "," + str(fn))
...
def do_epoch(model, data_loader, device, optimizer, loss_prob):
    model.train()
...
def do_test(model, data_loader, device):
    ...
    return accuracy, precision, recall, f1, true_positives, false_positives, true_negatives, false_negatives
