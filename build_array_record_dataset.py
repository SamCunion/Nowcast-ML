#tornet data builder, converts from slow files to array_record, which is much faster
...
dl_config = tfds.download.DownloadConfig(manual_dir=TORNET_ROOT) #sets the "download" path to a file system object, so no download.
#converts samples into TFDS array record format.
